...
def main() -> int:
    ...
    # Setup logging
    log_level = logging.DEBUG if ConversionParams.verbose else logging.INFO
    setup_logging(log_level)

    # Handle utility commands
    if ConversionParams.check_dependencies:
    ...
    if ConversionParams.version:
        from . import __version__
        print(f"fbx2glb version {__version__}")
        return 0

    # For utility commands, we don't need input_file
    if ConversionParams.check_dependencies or ConversionParams.version:
        return 0

    # Validate input file
    if not ConversionParams.input_file:
        logger.error("Input file is required")
        return 1
        
    if not os.path.exists(ConversionParams.input_file):
        logger.error(f"Input file '{ConversionParams.input_file}' does not exist")
        return 1

    # Check if FBX upgrade is needed
    if ConversionParams.upgrade_fbx:
        from .fbx_upgrader import check_fbx_upgrade_needed, upgrade_fbx_file
        
        needs_upgrade, version_info = check_fbx_upgrade_needed(ConversionParams.input_file)
        if needs_upgrade:
            logger.info(f"FBX file needs upgrade: {version_info}")
            logger.info("Attempting to upgrade FBX file...")
            
            # Create upgraded file path
            input_base = os.path.splitext(ConversionParams.input_file)[0]
            upgraded_file = f"{input_base}_upgraded.fbx"
            
            if upgrade_fbx_file(ConversionParams.input_file, upgraded_file, verbose=ConversionParams.verbose):
                logger.info(f"FBX upgrade successful: {upgraded_file}")
                # Update the input file path
                ConversionParams.input_file = upgraded_file
            else:
